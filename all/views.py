...
def index(request):
    def Humidity_Gauge(self):
        queryset = Humidity.objects.filter(reading_time__date=now).order_by('-id')[:1]
    ...
    def Temperature_Gauge(self):
        queryset = Temperature.objects.filter(reading_time__date=now).order_by('-id')[:1]
    ...
    def temp_daily_line(self):
        ...
        temp_daily_line_chart = gchart.LineChart(data_source, options={'title': "Daily Readings", 'vAxis':{'minValue':60,'maxValue':80}})
        return temp_daily_line_chart

    def humidity_daily_line(self):
        queryset = Humidity.objects.filter(reading_time__date=now)
