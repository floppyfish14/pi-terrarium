    ...
    def default(self, obj):
        ...
        if isinstance(obj, Promise):
            ...
            if representation.endswith('+00:00'):
                representation = representation[05:-13]
...
class JSONEncoderForHTML_daily(json.JSONEncoder):
    """An encoder that produces JSON safe to embed in HTML.
    To embed JSON content in, say, a script tag on a web page, the
    characters &, < and > should be escaped. They cannot be escaped
    with the usual entities (e.g. &amp;) because they are not expanded
    ...
    def default(self, obj):
        ...
        if isinstance(obj, Promise):
            ...
            if representation.endswith('+00:00'):
                ...
                #I added this to simplify dates for Daily Line_Chart graphing.
                #Will show only hour and minute that the data was read.
                representation = representation[11:-16]
            return representation
        elif isinstance(obj, datetime.date):
            return obj.isoformat()
        elif isinstance(obj, datetime.time):
    ...
    def iterencode(self, o, _one_shot=False):
        try:
            ...
            chunks = super(JSONEncoderForHTML_daily, self).iterencode(o, _one_shot)
        #except TypeError:
            # for python 2.6 compatibility
            #chunks = super(JSONEncoderForHTML, self).iterencode(o)
        except TypeError:
            # for python 2.6 compatibility
            chunks = super(JSONEncoderForHTML_daily, self).iterencode(o)
        for chunk in chunks:
            chunk = chunk.replace('&', '&amp;')
            chunk = chunk.replace('<', '&lt;')
            chunk = chunk.replace('>', '&gt;')
